...
def _runge_kutta_step(func, y0, f0, t0, dt, tableau):
    ...
    with tf.device(y0.device):
        k = tf.Variable(tf.zeros([*f0.shape, len(tableau.alpha) + 1], dtype=y0.dtype), trainable=False)
        k[..., 0].assign(f0)
        # k = _UncheckedAssign.apply(k, f0, (..., 0))

        for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)):
            ti = t0 + alpha_i * dt
            yi = y0 + tf.reshape(tf.matmul(k[..., :i + 1], tf.expand_dims(beta_i * dt, axis=-1)), f0.shape)
            print(f"y_{i} = {yi}")
            f = func(ti, yi)
            k[..., i + 1].assign(f)
        ...
        if not (tableau.c_sol[-1] == 0 and tf.reduce_all(tableau.c_sol[:-1] == tableau.beta[-1])):
            ...
            yi = y0 + tf.reshape(tf.matmul(k, tf.expand_dims(dt * tableau.c_sol, axis=-1)), f0.shape)

        k = tf.convert_to_tensor(k, dtype=k.dtype)

    y1 = yi
    f1 = k[..., -1]
    print(f"y1 : {y1}")
    # print(f"dt * c_error : {dt} * {tableau.c_error}")
    y1_error = tf.reshape(tf.matmul(k, tf.expand_dims(dt * tableau.c_error, -1)), y1.shape)
    print(f"y1_error : {y1_error[0]}")
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeODESolver):
    ...
    def __init__(self, func, y0, rtol, atol, first_step=None, safety=0.9, ifactor=10.0, dfactor=0.2,
        ...
        with tf.device(device):
            ...
            alpha = _checked_cast(tf.convert_to_tensor(self.tableau.alpha), y0)
            beta = [_checked_cast(tf.convert_to_tensor(b), y0) for b in self.tableau.beta]
            c_sol = _checked_cast(tf.convert_to_tensor(self.tableau.c_sol), y0)
            c_error = _checked_cast(tf.convert_to_tensor(self.tableau.c_error), y0)

            self.tableau = _ButcherTableau(alpha=alpha, beta=beta, c_sol=c_sol, c_error=c_error)
            self.mid = _checked_cast(tf.convert_to_tensor(self.mid), y0)
    ...
    def _advance(self, next_t):
        ...
        while next_t > self.rk_state.t1:
            ...
            n_steps += 1
            # print("rk common", "next_t", next_t, "t1", self.rk_state.t1)
    ...
    def _adaptive_step(self, rk_state):
        ...
        # print("rk common", "error_ratio", error_ratio, "accept_step", accept_step)
        # dtypes:
    ...
    def _interp_fit(self, y0, y1, k, dt):
        ...
        dt = _checked_cast(dt, y0)
        y_mid = y0 + tf.reshape(tf.matmul(k, tf.expand_dims(dt * self.mid, -1)), y0.shape)
        f0 = k[..., 0]
        f1 = k[..., -1]
